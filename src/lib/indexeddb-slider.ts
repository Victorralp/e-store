// IndexedDB Slider Management Utilities
...
// Extended interface for IndexedDB storage
interface IndexedDBSliderItem extends SliderItem {
  createdAt?: string;
  updatedAt?: string;
}
...
export const getAllSliderItems = async (): Promise<SliderItem[]> => {
  try {
    ...
    return new Promise((resolve, reject) => {
      request.onsuccess = () => resolve(request.result as SliderItem[]);
      request.onerror = () => reject(request.error);
    });
  } catch (error) {
...
export const createSliderItem = async (data: Omit<SliderItem, "id">): Promise<SliderItem> => {
  try {
    ...
    const item: IndexedDBSliderItem = {
      ...data,
    ...
    return new Promise((resolve, reject) => {
      getRequest.onsuccess = () => {
        if (getRequest.result) {
          const updatedItem: IndexedDBSliderItem = {
            ...getRequest.result,
            ...data,
            updatedAt: new Date().toISOString(),
          };
...
export const initializeDefaultSliderItems = async (): Promise<boolean> => {
  try {
    ...
    if (existingItems.length === 0) {
      const defaultSlides: Omit<SliderItem, "id">[] = [
        {
...
export const migrateFromLocalStorage = async (): Promise<boolean> => {
  try {
    ...
    if (localStorageData) {
      const sliderItems: SliderItem[] = JSON.parse(localStorageData);
